...
function IPFSPersistence () {
  if (!(this instanceof IPFSPersistence)) {
    return new IPFSPersistence()
  }
...
IPFSPersistence.prototype.storeRetained = function (packet, cb) {
...
IPFSPersistence.prototype.createRetainedStream = function (pattern) {
  return matchingStream([].concat(this._retained), pattern)
}

IPFSPersistence.prototype.createRetainedStreamCombi = function (patterns) {
  return matchingStream([].concat(this._retained), patterns)
}

IPFSPersistence.prototype.addSubscriptions = function (client, subs, cb) {
...
IPFSPersistence.prototype.removeSubscriptions = function (client, subs, cb) {
...
IPFSPersistence.prototype.subscriptionsByClient = function (client, cb) {
  var subs = null
  const stored = this._subscriptions.get(client.id)
  if (stored) {
    subs = []
...
IPFSPersistence.prototype.countOffline = function (cb) {
  return cb(null, this._trie.subscriptionsCount, this._clientsCount)
}

IPFSPersistence.prototype.subscriptionsByTopic = function (pattern, cb) {
  cb(null, this._trie.match(pattern))
}

IPFSPersistence.prototype.cleanSubscriptions = function (client, cb) {
  const trie = this._trie
  const stored = this._subscriptions.get(client.id)
...
IPFSPersistence.prototype.outgoingEnqueue = function (sub, packet, cb) {
  _outgoingEnqueue.call(this, sub, packet)
  process.nextTick(cb)
}

IPFSPersistence.prototype.outgoingEnqueueCombi = function (subs, packet, cb) {
...
IPFSPersistence.prototype.outgoingUpdate = function (client, packet, cb) {
...
IPFSPersistence.prototype.outgoingClearMessageId = function (client, packet, cb) {
  const clientId = client.id
  const outgoing = this._outgoing[clientId] || []
  var temp
...
IPFSPersistence.prototype.outgoingStream = function (client) {
...
IPFSPersistence.prototype.incomingStorePacket = function (client, packet, cb) {
  const id = client.id
...
IPFSPersistence.prototype.incomingGetPacket = function (client, packet, cb) {
  const id = client.id
  const store = this._incoming[id] || {}
  var err = null
...
IPFSPersistence.prototype.incomingDelPacket = function (client, packet, cb) {
  const id = client.id
  const store = this._incoming[id] || {}
...
IPFSPersistence.prototype.putWill = function (client, packet, cb) {
...
IPFSPersistence.prototype.getWill = function (client, cb) {
  cb(null, this._wills[client.id], client)
}

IPFSPersistence.prototype.delWill = function (client, cb) {
  const will = this._wills[client.id]
  delete this._wills[client.id]
  cb(null, will, client)
}

IPFSPersistence.prototype.streamWill = function (brokers) {
...
IPFSPersistence.prototype.getClientList = function (topic) {
...
IPFSPersistence.prototype.destroy = function (cb) {
  this._retained = null
  if (cb) {
    cb(null)
  }
...
module.exports = IPFSPersistence
